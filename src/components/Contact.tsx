...
const SOCIALS = [
  {
    ...
    hoverClass:
      "hover:border-white hover:bg-white hover:text-[#0a0a0a]",
    icon: (
...
export default function Contact() {
  return (
    <section className="flex min-h-full flex-col items-center justify-center px-6 pb-24 text-center sm:pb-0">
      ...
      <div className="mt-8 flex items-center gap-3">
        {SOCIALS.map((s) => (
          <a
            ...
            aria-label={s.label}
            title={s.label}
            className={`flex h-11 w-11 items-center justify-center rounded-full border border-white/[0.1] bg-white/[0.04] text-white/40 transition-all duration-200 ${s.hoverClass}`}
          >
            {s.icon}
          </a>
        ))}
      ...
      <p className="mt-16 text-xs text-white/15">© 2026 Jang Kyungjin<br/>본 프로젝트는 개인 포트폴리오 목적으로 공개되었습니다.<br/>코드를 무단으로 사용하지 말아주세요.</p>
    </section>
  );
}
