...
export default function Contact() {
  return (
    <section className="flex h-full flex-col items-center justify-center px-6 pb-24 text-center sm:pb-0">
      <span className="text-[10px] uppercase tracking-[0.35em] text-white/30">Contact</span>

      <h2 className="mt-1 max-w-3xl text-[clamp(1.75rem,5vw,3.5rem)] font-bold leading-[1.15] tracking-tight">
        같이 만들어가요
      </h2>

      <p className="mt-4 max-w-md text-md leading-1 text-white/40">
        하루 하루 성장하며 어제보다 나은 코드를 남기고 싶어요.
      </p>
      ...
      <div className="mt-8 flex items-center gap-3">
        {SOCIALS.map((s) => (
          <a
            ...
            aria-label={s.label}
            title={s.label}
            className="flex h-11 w-11 items-center justify-center rounded-full border border-white/[0.1] bg-white/[0.04] text-white/40 transition-all duration-200 hover:border-[var(--accent)]/40 hover:bg-white/[0.06] hover:text-[var(--accent)]"
          >
            {s.icon}
          </a>
        ))}
      ...
      <p className="mt-16 text-xs text-white/15">© 2026 장경진</p>
    </section>
  );
}
